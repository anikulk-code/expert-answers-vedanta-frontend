...
Source playlist: `PL2imXor63HtS4ewIKryBL4ZVeiaH8Ij4R` on the Vedanta Society
of New York channel. Currently ~172 episodes, covering through Chapter 15 as
of Feb 2026.
...
- `data/sarvapriyananda_gita_map.json` — the verse→video map. Stub today; run the ingestion script to produce the full version.
- `scripts/ingest_sarvapriyananda_gita.py` — YouTube Data API ingestion. Requires `YOUTUBE_API_KEY` env var. Run on demand; commit the resulting JSON.
- `docs/gita_feature_prompt.md` — this document.
- `docs/gita_preview.html` — standalone HTML preview of the verse detail page for design review. No integration; open in a browser.
